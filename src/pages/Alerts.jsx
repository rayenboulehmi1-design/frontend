    ...
    <div className="p-5 sm:p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Alerts</h1>
          </div>
          <p className="text-slate-500 dark:text-slate-400 text-sm">Get notified when new signals match your criteria.</p>
        </div>
        <button
      ...
      {showForm && (
        <motion.form
          ...
          className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 mb-6 space-y-4"
        >
          <div>
      ...
      {alerts.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-12 text-center">
          <Bell className="w-10 h-10 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
          <p className="text-slate-400 dark:text-slate-500 mb-4">No alerts configured yet.</p>
        ...
        <div className="space-y-3">
          {alerts.map((alert) => {
            ...
            return (
              <Link key={alert.id} to={demoLink(`/intelligence-feed?${alertParams.toString()}`)}>
                <motion.div
                  initial={{ opacity: 0, y: 12 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 flex items-center justify-between gap-4 hover:border-blue-200 dark:hover:border-blue-900 hover:shadow-sm transition-all"
                >
                  <div className="min-w-0">
                    <h3 className="font-semibold text-slate-900 dark:text-slate-100">{alert.name}</h3>

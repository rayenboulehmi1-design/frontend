...
export default function SignalCard({ signal, compact = false }) {
  ...
  if (compact) {
    return (
      <Link to={demoLink(`/opportunities/${signal.id}`)} className="block group">
        <div className="flex items-start gap-3 p-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-blue-200 dark:hover:border-blue-900 hover:shadow-md transition-all">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm font-medium text-slate-900 dark:text-slate-100 leading-snug truncate group-hover:text-blue-600 transition-colors">
              {signal.title}
            </p>
  ...
  return (
    <div className={`rounded-2xl border ${style.border} dark:border-slate-800 bg-white dark:bg-slate-900 p-5 hover:shadow-lg transition-all flex flex-col`}>
      ...
      <Link to={demoLink(`/opportunities/${signal.id}`)} className="group flex-1">
        <h3 className="font-semibold text-slate-900 dark:text-slate-100 leading-snug mb-3 group-hover:text-blue-600 transition-colors">
          {signal.title}
        </h3>
        {signal.summary && <p className="text-sm text-slate-500 dark:text-slate-400 mb-4 line-clamp-2">{signal.summary}</p>}
      </Link>

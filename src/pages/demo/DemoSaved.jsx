...
export default function DemoSaved() {
    ...
    <div className="p-5 sm:p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm">
          {saved.length > 0 ? `${saved.length} saved opportunity${saved.length !== 1 ? "s" : ""}` : "Bookmark signals to revisit them here."}
        </p>
      </div>
      {saved.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center">
          ...
          <Link to={demoLink("/dashboard")} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition-colors">Browse Intelligence</Link>
        </div>
      ) : (

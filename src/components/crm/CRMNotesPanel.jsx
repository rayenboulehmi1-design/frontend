          ...
          {notes.map((note) => (
            <div key={note.id} className="rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <button onClick={() => { deleteNote(note.id); onChanged(); }} aria-label="Delete note" className="text-slate-300 hover:text-rose-400 transition-colors shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 rounded">

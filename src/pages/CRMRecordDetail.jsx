  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="relative flex gap-4">
      <div className="flex flex-col items-center">
        <div className="w-9 h-9 rounded-full bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 flex items-center justify-center text-xs font-bold shrink-0 z-10">{number}</div>
      ...
      <div className="flex-1 pb-6 min-w-0">
        <button onClick={() => setOpen(!open)} aria-expanded={open} className="flex items-center gap-2 mb-2.5 flex-wrap w-full text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded-lg">
          ...
          <ChevronDown className={`w-3.5 h-3.5 text-slate-300 dark:text-slate-600 ml-auto transition-transform ${open ? "" : "-rotate-90"}`} />
        </button>
        {open && children}
      </div>
    </motion.div>
...
export default function CRMRecordDetail() {
  ...
  return (
    <div className="p-5 sm:p-8 max-w-4xl mx-auto">
      <Link to={demoLink("/crm")} className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 mb-6 transition-colors">
        <ArrowLeft className="w-4 h-4" /> Back to CRM
      </Link>
      ...
      <div className="flex items-start justify-between gap-4 mb-2 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">CRM Record</h1>
        </div>
      ...
      <p className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{record.title}</p>
      ...
      <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 mb-6">
        <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500 mb-2">Move to Stage</p>
              ...
              <React.Fragment key={stage}>
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-lg text-[11px] font-medium whitespace-nowrap border transition-colors ${
                    active ? `${meta.bg} ${meta.text} ${meta.border} font-bold` : "bg-white dark:bg-slate-900 text-slate-400 dark:text-slate-500 border-slate-100 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-600"
                  }`}
                >
                  {stage}
                </button>
